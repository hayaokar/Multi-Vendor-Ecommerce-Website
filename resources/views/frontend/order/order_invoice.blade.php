    ...
    <tr>
        <td>
            <p class="font" style="margin-left: 20px;">
                ...
                <strong>Address:</strong>{{ $order->country->name }}-{{ $order->city->name }}-{{ $order->adress }} <br>
...
<table width="100%">
    ...
    <tr class="font">
        <td align="center">
            <img src="data:image/png;base64,{{ base64_encode(file_get_contents(public_path($order_item->product->product_thambnail))) }}" height="45px;" width="45px;" alt="">
        </td>
        ...
        @if($order_item->size == null)
            <td align="center">...</td>
        @else
            <td align="center">{{$order_item->size}}</td>
        @endif
        ...
        @endif
        <td align="center">{{$order_item->product->code}}</td>
        <td align="center">{{$order_item->qty}}</td>
        <td align="center">${{$order_item->price}}</td>
        <td align="center">${{$order_item->price * $order_item->qty }}</td>
    </tr>
    @endforeach
    </tbody>
</table>
